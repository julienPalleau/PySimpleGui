"""
The Official Python GUI Programming with PySimpleGUI Course
Button Element - Lesson Part 4 - Simulated Buttons

Non-Button Elements that produce Button-like results:
    Text
    Image
    Graph
    Right Click Menu

Docs: http://www.PySimpleGUI.org
Built-in help: sg.sdk_help()
"""
import PySimpleGUI as sg

sg.set_options(font='Default 18')
sg.Print(font='Default 18', keep_on_top=True, size=(20, 14), location=(2292, 454))

example_number = 4


# =========================== 1 - Text Elements as Buttons =============================================================
def main_button_text_elmt():
    layout = [[sg.T('Which is the "real" button?')],
              [sg.Frame('', [[sg.Text(' Button? ', enable_events=True, background_color=sg.theme_button_color()[1],
                                      pad=(0, 4), k='-B1-')]], background_color=sg.theme_button_color()[1],
                        relief=sg.RELIEF_RAISED)],
              [sg.B('Button?', k='-B2-')]]

    window = sg.Window("Button Image Basics", layout, keep_on_top=True, use_default_focus=False)
    while True:
        event, values = window.read()
        if event in (sg.WIN_CLOSED, 'Exit'):
            break
        sg.Print(f'event = {event}', c='white on red', erase_all=True)
        sg.Print(*[f'{k} = {values[k]}' for k in values], sep='\n')
    window.close()


# =========================== 2 - Toggle Button Using Images ===========================================================
def main_toggle_basic():
    layout = [[sg.Image(data=toggle_btn_off, enable_events=True, key='-TOGGLE1-', metadata=False)],
              [sg.Button(image_data=toggle_btn_off, key='-TOGGLE2-', button_color=sg.TRANSPARENT_BUTTON, border_width=0,
                         metadata=False)],
              [sg.B('Exit')]]

    window = sg.Window("Toggle Button", layout, keep_on_top=True)
    while True:
        event, values = window.read()
        if event in (sg.WIN_CLOSED, 'Exit'):
            break
        sg.Print('event = {event}', c='white on red', erase_all=True)
        sg.Print(*[f'{k} = {values[k]}' for k in values], sep='\n')

        if event in ('-TOGGLE1-', '-TOGGLE2-'):
            window[event].metadata = not window[event].metadata
            if event == '-TOGGLE1-':
                window['-TOGGLE1-'].update(data=toggle_btn_on if window['-TOGGLE1-'].metadata else toggle_btn_off)
            else:
                window['-TOGGLE2-'].update(image_data=toggle_btn_on if window['-TOGGLE2-'].metadata else toggle_btn_off)

    window.close()


# =========================== 3 - Toggle Button Using Graph ============================================================
def main_toggle_graph():
    layout = [
        [sg.Graph((100, 40), (0, 40), (100, 0), k='-GRAPH-', enable_events=True)],
        [sg.Image(data=toggle_btn_off, enable_events=True, key='-TOGGLE1-', metadata=False)],
        [sg.Button(image_data=toggle_btn_off, key='-TOGGLE2-', button_color=sg.TRANSPARENT_BUTTON, border_width=0,
                   metadata=False)],
        [sg.B('Exit')]]

    window = sg.Window("Toggle Button", layout, keep_on_top=True, finalize=True)
    window['-GRAPH-'].draw_image(data=toggle_btn_off, location=(0, 0))
    while True:
        event, values = window.read()
        if event in (sg.WIN_CLOSED, 'Exit'):
            break

        sg.Print(f'event = {event}', c='white on red', erase_all=True)
        sg.Print(*[f'{k} = {values[k]}' for k in values], sep='\n')

        if event in ('-TOGGLE1-', '-TOGGLE2-', '-GRAPH-'):
            window[event].metadata = not window[event].metadata
            if event == '-TOGGLE1-':
                window[event].update(data=toggle_btn_on if window['-TOGGLE1-'].metadata else toggle_btn_off)
            elif event == '-GRAPH-':
                window[event].erase()
                window[event].draw_image(data=toggle_btn_on if window[event].metadata else toggle_btn_off,
                                         location=(0, 0))
            else:
                window['-TOGGLE2-'].update(image_data=toggle_btn_on if window['-TOGGLE2-'].metadata else toggle_btn_off)
    window.close()


# =========================== 4 - Right Click Menu instead of Buttons ==================================================

def main_right_click_menu():
    layout = [[sg.Graph((160, 160), (0, 0), (160, 160), key='-GRAPH-')]]

    window = sg.Window('Desktop Widget', layout, no_titlebar=True, grab_anywhere=True, margins=(0, 0), keep_on_top=True,
                       right_click_menu=sg.MENU_RIGHT_CLICK_EDITME_VER_SETTINGS_EXIT, finalize=True)

    window['-GRAPH-'].draw_text('CPU', (80, 40), font='_ 20', text_location=sg.TEXT_LOCATION_CENTER,
                                color=sg.theme_button_color()[0])
    window['-GRAPH-'].draw_text('41%', (90, 90), font='_ 40', text_location=sg.TEXT_LOCATION_CENTER,
                                color=sg.theme_button_color()[0])

    while True:
        event, values = window.read()
        if event in (sg.WIN_CLOSED, 'Exit'):
            break

        sg.Print(f'event = {event}', c='white on red', erase_all=True)
        sg.Print(*[f'{k} = {values[k]}' for k in values], sep='\n')
    window.close()


if __name__ == "__main__":
    toggle_btn_off = b''
    toggle_btn_on = b''

    example = [main_button_text_elmt, main_toggle_basic, main_toggle_graph, main_right_click_menu]
    example[example_number - 1]()
